...
class eCelebV_Dataset(Dataset):
    def __init__(self, cfg, root, is_train, transform=None):
        self.Image_size = cfg.MODEL.IMG_SIZE
        self.is_train = is_train
        self.root = cfg.eCelebV.ROOT
        self.number_landmarks = cfg.eCelebV.NUM_POINT
        self.flip_index = np.genfromtxt("Mirror.txt", dtype=int, delimiter=',')

        self.Fraction = cfg.eCelebV.FRACTION
        self.Translation_Factor = cfg.eCelebV.TRANSLATION
        self.Rotation_Factor = cfg.eCelebV.ROTATION
        self.Scale_Factor = cfg.eCelebV.SCALE
        self.Occlusion_Mean = cfg.eCelebV.OCCLUSION_MEAN
        self.Occlusion_Std = cfg.eCelebV.OCCLUSION_STD
        self.Flip = cfg.eCelebV.FLIP
        self.Occlusion = cfg.eCelebV.OCCLUSION
        self.Transfer = cfg.eCelebV.CHANNEL_TRANSFER

        self.Heatmap_size = cfg.MODEL.HEATMAP

        self.Data_Format = cfg.eCelebV.DATA_FORMAT
        self.Event_Path = os.path.join(cfg.eCelebV.ROOT, cfg.DATASET.REPR)
        self.Event_Repr = cfg.DATASET.REPR

        self.Transform = transform

        with open(os.path.join(root, 'train_val_test_split_multilabel_mod.json'), 'r') as splits_file:
            splits = json.load(splits_file)
            if self.is_train == 'train':
                self.split = splits['train']
            elif self.is_train == 'val':
                self.split = splits['validation']
            elif self.is_train == 'test':
                self.split = splits['test']
            else:
                raise ValueError(f"Unknown split value:{self.is_train} Please use one of 'train', 'val', or 'test'.")

        self.annotation_dir = os.path.join(root, 'ecelebv_gt_frame_max')
        ...
        for file_name in txt_files:
            ...
            if vid_name in self.split:
                ...
                for temp_info in lines:
                    ...
                    img_path = os.path.join(self.root, "rgb_frame_max", vid_name, temp_info[-1])
